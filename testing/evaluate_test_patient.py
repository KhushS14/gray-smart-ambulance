...
def plot_metrics_comparison(metrics: dict, save_path: str = "evaluation/metrics_comparison.png"):
    ...
    ax.set_title('Performance Metrics - Smart Ambulance Alert System', fontsize=14, pad=20)
    ax.legend()
...
def print_detailed_report(metrics: dict, latency_stats: dict):
    ...
    if metrics['recall'] >= 0.90 and metrics['false_alert_rate'] <= 0.20:
...
if __name__ == "__main__":
    ...
    print(f"✅ Loaded {len(df_clean)} vitals + {len(df_risk)} risk windows")
    
    # Define ground truth
    print("Defining ground truth labels...")
    ...
    # Print report
    print_detailed_report(metrics, latency_stats)
    ...
    results_df.to_csv("evaluation/metrics_results.csv", index=False)
    print("✅ Metrics saved to evaluation/metrics_results.csv")
    
    df_risk.to_csv("evaluation/risk_with_ground_truth.csv", index=False)
    print("✅ Risk data saved to evaluation/risk_with_ground_truth.csv")
